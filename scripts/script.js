...
function Lens(p1, p2 = [-1, -1], focalpoint = 300) {
  ...
  Object.defineProperty(this, "f1", {
    get: () => {
      ...
      if (b == 0) {
        if (this.p1[0] < this.p2[0]) normal = [0, -1];
        else normal = [0, 1];
      }
...
function recursiveRaytrace(
  ...
  if (remainingBounces > 0) {
    ...
    if (intersection.dist < Infinity) {
      ...
      if (intersection.dir == null) return bounces;
      return recursiveRaytrace(
        intersection.p,
        intersection.dir,
        remainingBounces - 1,
        bounces
  ...
  bounces.push([
    position[0] + direction[0] * 1e6,
    position[1] + direction[1] * 1e6,
  ]);
  return bounces;
}
...
function circleIntersection(rayOrigin, direction, mirror) {
  ...
  function validateDirection(intersect) {
    ...
    return (
      Math.sign(intersect[0] - rayOrigin[0]) == Math.sign(direction[0]) &&
  ...
  if (intersections.length > 1)
    if (
      dist2(vSub(intersections[1], rayOrigin)) <
      dist2(vSub(intersections[0], rayOrigin))
    )
...
function lineIntersects(rayOrigin, direction, mirror) {
  ...
  yres = a * xres;
  if (b == 0)
    //fixes edge cases with rounding errors
    yres = s1[1];
  ...
  if (xres <= Math.max(s1[0], s2[0]) && xres >= Math.min(s1[0], s2[0]))
    if (yres <= Math.max(s1[1], s2[1]) && yres >= Math.min(s1[1], s2[1]))
      if (
        Math.sign(xres) == Math.sign(direction[0]) &&
